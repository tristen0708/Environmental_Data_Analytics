...
Peter.Paul <- read.csv("./Data/Processed/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv")
...
Peter.Paul.gathered$sampledate <- as.Date(Peter.Paul.gathered$sampledate, format = "%Y-%m-%d")
...
phosphorus <- ggplot(Peter.Paul, aes(x = po4, y = tp_ug, color = lakename)) +
  scale_color_manual(values=c("#c994c7", "#225ea8")) +  #change colors
  geom_point() +
  xlim(0, 50) +
  ylim(0, 150) +
  geom_smooth(method = lm, color = 'black') +
  labs(x = "Phosphate" , y = "Total Phosphorus, μgrams/L", fill = "lakename", color = "Lake Name")
print(phosphorus)  
...
nutrients <- ggplot(Peter.Paul.gathered) +
  geom_point(aes(x = nutrient, y = concentration, color = depth)) +
  facet_grid(concentration ~ sampledate) + 
...
nutrients <- ggplot(EPA.ecotox) +
  ...
  labs(y = "Concentration", color = "Chemical Name")
print(nutrients)  
