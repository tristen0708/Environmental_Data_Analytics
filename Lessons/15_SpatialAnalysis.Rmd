...
plot(huc8_sf['SUB'], 
     graticule = st_crs(huc8_sf), axes = TRUE) # Add a graticule
...
state_sf <- counties_sf %>% 
  group_by(STATEFP) %>%  #We don't actually need this line since we just have NC data
...
huc0302_int <- st_intersection(counties_sf_utm, huc0302_counties)
  
#Map the product, showing values by land area
mapview(huc0302_int['ALAND'], legend=FALSE)
```
