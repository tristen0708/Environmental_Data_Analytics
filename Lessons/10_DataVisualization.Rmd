...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +  #sets us axes
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"),  #added green rectangle for 0 - 50 y axis
            ymin = 0, ymax = 50, fill = "green") +
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), 
            ymin = 50, ymax = 100, fill = "yellow") +
  geom_point() + #geom point adds all my data points
  geom_text(x = as.Date("2019-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") + #added word good at height 45, hjust - offsets from center of x point specified
  ...
  scale_x_date(limits = as.Date(c("2017-01-01", "2018-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") + #rewrites dates from 2017-01 to Jan 17 - added ticks to every 2 months
  ylab(expression("O"[3]* " AQI Value")) + #add label for y axis 
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #makes data angled
print(O3plot7)  
...
NvsP2 <-
  ...
  geom_point(alpha = 0.8, size = 3, color = "black") +
  #xlab("Total Phosphorus, μgrams") +
  #ylab("Total Nitrogen, μgrams") +
  labs(x= "Total Phosphorus, μgrams/L", y="Total Nitrogen, μgrams/L", fill= "Depth, m", shape= "Lake Name") +
...
Nutrientplot <-
  ...
  geom_boxplot() +
  labs(x= NULL, y="Concentration μgrams/L", color= "Nutrient") +
# place your additional edits here
  scale_y_continuous(expand = c(0, 0)) +
  #scale_color_brewer(palette = "YlGnBu") +
  scale_color_manual(values = c("#7fcdbb", "#41b6c4", "#1d91c0", "#225ea8", "#0c2c84"), labels = c("Ammonium", "Nitrite", "Phosphate", "Total Nitrogen", "Total Phosphorus")) +
