...
> ANSWER: When depth is zero our irradiance value will equal 486.818. A one unit increase in depth will result in a 96 unit decrease in irradiance (linear regression; R-squared = 0.3091, df = 15449, p < 0.001).
~Irradiance is negatively correlated with depth.~
...
irradiancebydepth_pretty <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point(alpha=0.2) +  #lower alpha value makes more transparent
    scale_y_log10() +
    geom_smooth(method = 'lm', se = FALSE, color = "pink") +
    labs(x = "Depth, m", y = "Water Irradiance, units") +
    scale_x_continuous(breaks = seq(0, 10, 2)) +
    annotate("text", x = 10, y = 10000, label = "y = 6.22 - 0.74x + 0.96")
print(irradiancebydepth_pretty)
...
# Make the graph attractive
TNplot.cute <- ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = tn_ug, color = lakename)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  xlim(0, 10) +
  labs(x = "Depth, m", y = "Total Nitrogen, micrograms/L", color = "Lake Name")
print(TNplot.cute)
