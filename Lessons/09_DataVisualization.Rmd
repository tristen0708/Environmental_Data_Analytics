...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3)  #doing this puts it all in rows stacked on top of one another - by default it will be three different columns side by side
...
PMplot.faceted2 <-
  ...
  geom_point() +
  facet_grid(Site.Name ~ year) #tilda means by
...
O3plot.Blackstone <- 
  ...
  geom_point() +
  geom_line() #useful to know swings
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm) + ##if you exclude (method = lm) then it will do a best fit line that is                                not necessarily linear
  geom_abline(aes(slope = 16, intercept = 0))  # lyt=# - line type goes outside of aes
print(NvsP)
...
Nutrientplot <- 
  ...
  geom_bar(aes(x = lakename, y = concentration, fill = as.factor(nutrient)), # we use fill so it colors the entire bar - if you used color = it only does the outline of the bar and colors inside gray
           position = "dodge", # dodge gives each color their own bar horizontally - otherwise they stack up - only two bars for each late
           stat = "summary", 
           fun.y = "mean")             # telling it to do mean values of concentration data
print(Nutrientplot)

Nutrientplot2 <- 
  ggplot(PeterPaul.nutrient.summaries, aes(x = lakename, y = mean, fill = as.factor(nutrient))) + #
  geom_bar(stat = "identity", position = "dodge") + # stat = identity which is telling it to chose the value specific in the mean column
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Dont want to use fill it colors in the box but not border
print(Nutrientplot3)
...
p1 <- 
  ggplot(EPAair.summaries, aes(x = year, y = mean, fill = as.factor(Site.Name)))  +
