...
PeterPaul.summertemp <- 
  PeterPaul.chem %>%
  select(lakename:temperature_C) %>% #choose columns that are important
  #filter for Julian days in June-August and surface measurements
  filter(daynum > 151 & daynum < 243 & depth == 0 ) %>% #getting surface temp and only summer
  #add a "week" column to represent seasonality
  mutate(Week = week(sampledate)) %>% #command from lubridate
...
#this doesnt account for variability within the season
```
...
TempTest.auto <- lme(data = PeterPaul.summertemp,
                     temperature_C ~ sampledate * lakename, #fixed effects ANCOVA
                     random = ~1|Week) #random effect
TempTest.auto
...
ggplot(PeterPaul.summertemp, aes(x = sampledate, y = temperature_C, color = lakename)) +
  ...
  labs(x = "Sample Data", y = "Temperature, C", color = "") +
  #stat_regline_equation(label.x = 3, label.y = 32)
  geom_text(label = "y = 21 + 0.000099x", x = 2, y = 2)
