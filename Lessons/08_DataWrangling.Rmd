...
colnames(NTL.phys.data.PeterPaul) #we could choose to rename to include units
dim(NTL.phys.data.PeterPaul)
summary(NTL.phys.data.PeterPaul$comments)
class(NTL.phys.data.PeterPaul$sampledate) #before we saved this column as a date - but when we exported as a csv R forgot - so we need to format
...
NTL.nutrient.data.PeterPaul <- filter(NTL.nutrient.data, lakename == "Paul Lake" | lakename == "Peter Lake")  #filter chooses ROWS ; select chooses COLUMNS
...
# Remove columns that are not of interest for analysis - could use "drop.levels"
...
```{r}
# Gather nutrient data into one column  - gather and spread are with tidyr
...
interval(NTL.phys.nutrient.data.PeterPaul$sampledate[1], NTL.phys.nutrient.data.PeterPaul$sampledate[23372]) #this may not be as reproducible
...
NTL.PeterPaul.summaries <- 
  ...
  filter(depth) %>%
  group_by(lakename) %>%
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>%  #! means not
...
#(NTL.PeterPaul.summaries, row.names = FALSE, 
